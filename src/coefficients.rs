...
impl Coefficients {
    pub fn new<T>(data: [u8; 22]) -> Result<Self, CustomError<T>> {
        let instance = Self {
            ac1: i16::from_be_bytes(data[0..2].try_into().unwrap()),
            ac2: i16::from_be_bytes(data[2..4].try_into().unwrap()),
            ac3: i16::from_be_bytes(data[4..6].try_into().unwrap()),

            ac4: u16::from_be_bytes(data[6..8].try_into().unwrap()),
            ac5: u16::from_be_bytes(data[8..10].try_into().unwrap()),
    ...
    pub fn calculate_temperature(&self, uncompensated_temperature: i32) -> (f32, i32) {
        let x1 = ((uncompensated_temperature - self.ac6 as i32) * self.ac5 as i32) >> 15;
    ...
    pub fn calculate_pressure(&self, b5: i32, uncompensated_pressure: i32, resolution: Resolution) -> i32 {
        ...
        let b3: i32 = (((self.ac1 as i32 * 4 + (x3 as i32)) << resolution) + 2) / 4;
        ...
        let b7: u32 = (uncompensated_pressure - b3) as u32 * (50000 >> resolution);
        let p = if b7 < 0x80000000 { (b7 << 1) / b4 } else { (b7 / b4) << 1 } as i32;
